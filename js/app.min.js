...
// Установка позиции — нижний правый угол
scrollToTopImg.style.position = 'fixed';
scrollToTopImg.style.bottom = '20px'; // отступ от низа
scrollToTopImg.style.right = '20px';  // отступ от правого края
scrollToTopImg.style.zIndex = '1000'; // поверх других элементов

// Вращение по оси Y при наведении
function rotateYOnHover(duration = 800, fullRotation = 360) {
  let animationFrame;
  let start = null;
  ...
  function animate(time) {
    ...
    const angle = fullRotation * progress;
    scrollToTopImg.style.transform = `rotateY(${angle}deg)`;
  ...
  scrollToTopImg.addEventListener('mouseenter', () => {
    cancelAnimationFrame(animationFrame);
    start = null;
    requestAnimationFrame(animate);
  });

  scrollToTopImg.addEventListener('mouseleave', () => {
    cancelAnimationFrame(animationFrame);
    scrollToTopImg.style.transform = 'rotateY(0deg)';
  });
}

rotateYOnHover();

  // Получение разницы между полной шириной окна и шириной видимой части документа (учитывает полосу прокрутки)
  function e() {
  ...
  function t(e, t = !0, s = !0) {
   ...
   // Проверка, является ли объект обычным объектом (не массивом и не null)
  function s(e) {
    return (
      null !== e &&
      "object" == typeof e &&
  ...
  // Глубокое объединение объектов (рекурсивно)
  function i(e = {}, t = {}) {
  ...
  // Получение объекта document с заглушками
  function r() {
    const e = "undefined" != typeof document ? document : {};
    return i(e, a), e;
  }
  // Заглушки для объекта window
  const n = {
    document: a,
    navigator: { userAgent: "" },
    location: {
  ...
  // Получение объекта window с заглушками
  function l() {
  ...
  // Глубокое объединение объектов с учётом специфики Swiper
  function c(...e) {
    const t = Object(e[0]),
  ...
  // Плавная прокрутка для Swiper в режиме CSS
  ...
  // Получение индекса элемента среди его братьев
  function v(e) {
    let t,
      s = e;
    if (s) {
  ...
  // Получение всех родительских элементов, удовлетворяющих селектору
  function b(e, t) {
    const s = [];
  ...
  // Кэширование результата проверки доступности smoothScroll и тач-событий
  let E, w, S;
  function y() {
    return (
      E ||
  ...
  const C = {
      ...
      // Подписка на событие с автоматическим удалением после первого вызова
      once(e, t, s) {
        const i = this;
        if (!i.eventsListeners || i.destroyed) return i;
        if ("function" != typeof t) return i;
      ...
      // Отписка от конкретного события или всех обработчиков события
      off(e, t) {
        const s = this;
      ...
      // Вызов события (исполнение всех соответствующих обработчиков)
      emit(...e) {
        const t = this;
        if (!t.eventsListeners || t.destroyed) return t;
    ...
    // Предзагрузка ближайших слайдов вокруг активного
    A = (e) => {
      if (!e || e.destroyed || !e.params) return;
      let t = e.params.lazyPreloadPrevNext;
      const s = e.slides.length;
      if (!s || !t || t < 0) return;
      t = Math.min(t, s); // Ограничиваем preload значением количества слайдов
      const i =
        ...
        r = a + i - 1;
      if (e.params.rewind)
        // Предзагрузка слайдов с учетом цикличности (rewind)
    ...
    // Слайдер
    T = {
      // Обновляет размеры слайдера
      updateSize: function () {
        const e = this;
        let t, s;
        const i = e.el; // DOM-элемент слайдера
        // Определение ширины
        (t =
          ...
          (s =
            void 0 !== e.params.height && null !== e.params.height
              ? e.params.height
              : i.clientHeight),
          // Если ширина равна нулю и слайдер горизонтальный или высота равна нулю и слайдер вертикальный — выход
          (0 === t && e.isHorizontal()) ||
            (0 === s && e.isVertical()) ||
            // Вычитание внутренних отступов (padding) из ширины и высоты
            ((t =
              t -
            ...
            // Защита от некорректных значений (NaN)
            Number.isNaN(t) && (t = 0),
            Number.isNaN(s) && (s = 0),
            // Присваиваем полученные значения объекту
            Object.assign(e, {
              width: t,
              height: s,
      ...
      updateSlides: function () {
        const e = this;
        // Преобразование CSS-свойств в зависимости от ориентации слайдера
        function t(t) {
        ...
        // Получение числового значения CSS-свойства
        function s(e, s) {
        ...
        // Массивы для хранения размеров и позиций
        let g = []; // snapGrid
        const v = [], // slidesGrid
          b = []; // slidesSizesGrid
        // Смещения в начале и конце
        ...
        // Храним старые длины сеток
        const S = e.snapGrid.length,
          y = e.slidesGrid.length;
        // Расстояние между слайдами
        let C = i.spaceBetween,
          _ = -E, // текущая позиция начала
          x = 0, // ширина предыдущего слайда
          A = 0; // индекс текущего слайда
        if (void 0 === n) return;
        // Если spaceBetween задан в процентах
        "string" == typeof C &&
          C.indexOf("%") >= 0 &&
          (C = (parseFloat(C.replace("%", "")) / 100) * n),
          (e.virtualSize = -C),
          // Сброс отступов для слайдов
          c.forEach((e) => {
            l ? (e.style.marginLeft = "") : (e.style.marginRight = ""),
              (e.style.marginBottom = ""),
              (e.style.marginTop = "");
          }),
          // Установка CSS переменных для центрированных слайдов
          i.centeredSlides &&
            i.cssMode &&
        ...
        // Обработка breakpoints при slidesPerView: 'auto'
        const F =
        ...
        for (let a = 0; a < h; a += 1) {
          ...
          if (
            ...
            if ("auto" === i.slidesPerView) {
              ...
              if (
                // Удаление трансформаций для получения корректных размеров
            ...
            c[a] && (c[a].swiperSlideSize = M),
              b.push(M),
              // Вычисление snapGrid и slidesGrid
              i.centeredSlides
                ? ((_ = _ + M / 2 + x / 2 + C),
                  0 === x && 0 !== a && (_ = _ - n / 2 - C),
                  0 === a && (_ = _ - n / 2 - C),
        ...
        if (
          // Учитываем slidesOffsetAfter
          ((e.virtualSize = Math.max(e.virtualSize, n) + w),
          // Установка размеров wrapper
          l &&
            o &&
          ...
          // Обрезка snapGrid, если не включен centeredSlides
          !i.centeredSlides)
        ) {
          const t = [];
        ...
        // Поддержка loop в виртуальном режиме
        if (u && i.loop) {
          const t = b[0] + C;
        ...
        // Применение отступов между слайдами
        ...
        // Корректировка для centerInsufficientSlides
        ...
        if (
          // Сохраняем результаты в объекте Swiper
          (Object.assign(e, {
            slides: c,
            snapGrid: g,
            slidesGrid: v,
          ...
          // Обновление CSS переменных при cssMode
        ...
        if (
          ...
          // Добавление/удаление класса backface-hidden
      ...
      // Обновление автоматической высоты контейнера слайдов
      updateAutoHeight: function (e) {
        ...
        let a,
          r = 0;
        // Установка transition, если задано число или true
        "number" == typeof e
          ? t.setTransition(e)
          : !0 === e && t.setTransition(t.params.speed);
        // Функция для получения слайда по индексу или виртуальным данным
        const n = (e) => (i ? t.getSlideIndexByData(e) : t.slides[e]);
        // Обработка нескольких слайдов в зависимости от режима отображения
        ...
        // Вычисление максимальной высоты среди выбранных слайдов
        for (a = 0; a < s.length; a += 1)
          // Если значение не передано — вычислить его
          if (void 0 !== s[a]) {
            const e = s[a].offsetHeight;
            r = e > r ? e : r;
          }
        // Установка высоты обертки
        (r || 0 === r) && (t.wrapperEl.style.height = `${r}px`);
      },
      // Обновление отступа слайда относительно начала обертки
      ...
      // Обновление прогресса каждого слайда (для анимаций и видимости)
      updateSlidesProgress: function (e = (this && this.translate) || 0) {
        const t = this,
          s = t.params,
          { slides: i, rtlTranslate: a, snapGrid: r } = t;
        if (0 === i.length) return;
        // Обновление отступов, если ещё не установлены
        void 0 === i[0].swiperSlideOffset && t.updateSlidesOffset();
        let n = -e;
        a && (n = e),
          // Очистка текущих видимых слайдов
          i.forEach((e) => {
        ...
        // Проверка, видим ли слайд на экране
          ((c >= 0 && c < t.size - 1) ||
            (p > 1 && p <= t.size) ||
            (c <= 0 && p >= t.size)) &&
            ...
            // Прогресс для анимаций
            (l.progress = a ? -u : u),
            (l.originalProgress = a ? -d : d);
        }
      },
      // Обновление общего прогресса карусели
      updateProgress: function (e) {
        ...
        const o = r,
          u = n;
        // Если нет расстояния прокрутки — прогресс = 0
        if (0 === i) (a = 0), (r = !0), (n = !0);
        else {
        ...
        // Поддержка циклической прокрутки
        if (s.loop) {
        ...
        // Присвоение рассчитанных значений
        Object.assign(t, {
          progress: a,
          progressLoop: l,
          isBeginning: r,
        ...
        // Обновление прогресса слайдов, если нужно
          (s.watchSlidesProgress || (s.centeredSlides && s.autoHeight)) &&
            t.updateSlidesProgress(e),
          // Генерация событий перехода состояний
      ...
      updateSlidesClasses: function () {
        const e = this,
          ...
          // Утилита для поиска слайда по селектору
          n = (e) => m(i, `.${s.slideClass}${e}, swiper-slide${e}`)[0];
        let l;
        if (
          // Удаляем классы активности со всех слайдов
          (t.forEach((e) => {
            e.classList.remove(
              s.slideActiveClass,
              s.slideNextClass,
        ...
        // Если включен виртуальный режим
          if (s.loop) {
            let t = a - e.virtual.slidesBefore;
            // Обеспечиваем круговой индекс
            t < 0 && (t = e.virtual.slides.length + t),
              t >= e.virtual.slides.length && (t -= e.virtual.slides.length),
              (l = n(`[data-swiper-slide-index="${t}"]`));
          } else l = n(`[data-swiper-slide-index="${a}"]`);
          // Обычный слайд по индексу
        else l = t[a];
        if (l) {
          // Добавляем класс активного слайда
      ...
      updateActiveIndex: function (e) {
        ...
        let o,
          u = e;
        // Преобразование индекса виртуального слайда в реальный
        const d = (e) => {
        ...
        // Если индекс не передан, вычисляем его по позиции прокрутки
        if (
          (void 0 === u &&
            (u = (function (e) {
        ...
        // Определение snapIndex (для пагинации)
          o = i.indexOf(s);
        else {
          const e = Math.min(a.slidesPerGroupSkip, u);
          o = e + Math.floor((u - e) / a.slidesPerGroup);
        }
        // Ограничение snapIndex
        if ((o >= i.length && (o = i.length - 1), u === r)) // Если активный индекс не изменился
          return (
            o !== l && ((t.snapIndex = o), t.emit("snapIndexChange")),
            // Обновляем только realIndex для виртуальных слайдов
            void (
              t.params.loop &&
              t.virtual &&
      ...
      updateClickedSlide: function (e) {
        const t = this,
          s = t.params,
          // Находим ближайший элемент-слайд от места клика
          i = e.closest(`.${s.slideClass}, swiper-slide`);
        let a,
          r = !1;
        // Находим индекс слайда в массиве
        if (i)
